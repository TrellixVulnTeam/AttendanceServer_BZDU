...
import datetime
# to Getting distance between two points based on latitude/longitude
...
from .serializer import courseSerializer, attendSerializer, lessonSerializer, request_attend, join_course, \
    ...
    request_new_lesson,request_attendance_location
...
class create_lesson_location(APIView):
    def post(self, request):
        ...
        date_lesson = mdata.data['date_lesson']
        name_lesson = mdata.data['name_lesson']
        latlocation = mdata.data['latlocation']
        longlocation = mdata.data['longlocation']
        response_True = {
            "ok": True,
            "massage": "Thành công"
        }
        ...
        if (user.user_type == 'lecturer'):
            ...
            if (date == None):
                course.lessons.create(name=name_lesson,
...
#     attandance for lesson with location
class attendance_location(APIView):
    def post(self,request):
        response_True = {
            "ok": True,
            "massage": "Thành công"
        ...
        mdata=request_attendance_location(data=request.data)
        if not mdata.is_valid():
            return Response(status=status.HTTP_400_BAD_REQUEST)
        code_user = mdata.data['code_user']
        code_course = mdata.data['code_course']
        ...
        # location of lesson
        latlocationkey=lesson.latlocation
        longlocationkey=lesson.longlocation
        print("Result1:", latlocation)
        print("Result2:", longlocation)
        print("Result3:", latlocationkey)
        print("Result4:", longlocationkey)
        #
        # approximate radius of earth in km
        R = 6373.0
        lat1 = radians(latlocation)
        lon1 = radians(longlocation)
        lat2 = radians(latlocationkey)
        lon2 = radians(longlocation)
        dlon = lon2 - lon1
        dlat = lat2 - lat1
